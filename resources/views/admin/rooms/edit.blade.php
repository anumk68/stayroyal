                        ...
                        @if ($rooms->room_images)
                            <div class="mt-2 d-flex flex-wrap gap-2">
                                @foreach (json_decode($rooms->room_images, true) as $img)
                                    <img src="{{ asset('public/' . $img) }}" width="100" class="img-thumbnail">
                                @endforeach
                            </div>
                        @endif
                    </div>
    ...
    <div id="amenities_wrapper">
        ...
        @foreach ($amenities as $i => $amenity)
            <div class="row mb-2 amenity-item">
                <div class="col-md-5">
                    <input type="file" name="amenities[{{ $i }}][icon]" class="form-control">
                    @if (!empty($amenity['icon']))
                        <img src="{{ asset('storage/app/public/' . $amenity['icon']) }}" alt="icon" class="mt-2" width="40">
                        <input type="hidden" name="amenities[{{ $i }}][old_icon]" value="{{ $amenity['icon'] }}">
                    @endif
                </div>
                <div class="col-md-5">
                    <input type="text" name="amenities[{{ $i }}][text]" value="{{ $amenity['text'] ?? '' }}" class="form-control" placeholder="Amenity text">
                </div>
                <div class="col-md-2 d-flex align-items-center">
                    <button type="button" class="btn btn-danger btn-sm" onclick="removeAmenity(this)">Delete</button>
                </div>
            </div>
        @endforeach
    </div>
